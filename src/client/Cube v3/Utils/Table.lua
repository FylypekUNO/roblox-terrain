...
function TableUtils.MapArray<T1, T2>(t: { T1 }, callback: (T1, number, { T1 }) -> T2): { T2 }
	local result = {}
...
function TableUtils.MapObject<T1, T2, T3, T4>(t: { [T1]: T2 }, callback: (T2, T1, { [T1]: T2 }) -> (T3, T4)): { [T3]: T4 }
	local result = {}
...
function TableUtils.Sort<T>(t: { T }, callback: (T, T) -> boolean): { T }
	local result = TableUtils.Sum(t)

	table.sort(result, callback)
