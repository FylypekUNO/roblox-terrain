...
local function createUI(points: { Point.Point }): ()
	-- Create ScreenGui
	...
	for i, point in ipairs(points) do
		...
		local function updateSlider(inputY)
			...
			local displayValue = math.floor(normalizedY * 100) / 100
			valueLabel.Text = string.format("%.2f", displayValue)

			-- Direct connection: Update the point's density value
			point.Density = normalizedY
		end
