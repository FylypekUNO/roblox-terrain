...
local function heightCallback(x: number, z: number, scale: number, seed: number): number
	...
	if h > 3 then
		return WORLD_START_POSITION.Y + h
	else
		return WORLD_START_POSITION.Y
	end
end
...
for chunkZ = -8, 8 do
	for chunkY = -2, 3 do
		for chunkX = -8, 8 do
			...
			local chunkStartPos = WORLD_START_POSITION
				+ Vector3.new(chunkX, chunkY, chunkZ) * (CHUNK_SIZE - 1) * BLOCK_PHYSICAL_SIZE
				- Vector3.one * BLOCK_PHYSICAL_SIZE / 2
